...
export const useAudioPlayer = ({
  ...
  const [volume, setVolume] = useState(() => {
    const saved = localStorage.getItem("pancake_volume");
  ...
  useEffect(() => {
    localStorage.setItem("pancake_volume", volume.toString());
  }, [volume]);
  ...
  const playNext = useCallback(
    (isAuto = false) => {
      ...
      // Repeat One handling for auto-advancing
      if (isAuto && rep === "one" && idx !== null) {
        // Reset current time and ensure it's playing
