...
export const useQueue = ({ enableGapless }: UseQueueProps) => {
  ...
  const [shuffle, setShuffle] = useState(() => {
    return localStorage.getItem("pancake_shuffle") === "true";
  });
  const [repeat, setRepeat] = useState<RepeatMode>(() => {
    return (localStorage.getItem("pancake_repeat") as RepeatMode) || "none";
  });

  // Sync to localStorage
  useEffect(() => {
    localStorage.setItem("pancake_shuffle", shuffle.toString());
  }, [shuffle]);

  useEffect(() => {
    localStorage.setItem("pancake_repeat", repeat);
  }, [repeat]);
        ...
        return prev.map((t) => {
          ...
          if (update && update.success && update.metadata) {
            return {
              ...
              cover_image: update.metadata.cover_image,
            };
