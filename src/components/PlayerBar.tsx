...
const PlayerBar: React.FC<PlayerBarProps> = ({
  ...
  const getCoverSrc = (cover?: string) => {
    ...
    if (cover.length > 512 || cover.startsWith("data:")) {
  ...
  return (
    <div className="h-24 bg-gray-900/90 backdrop-blur-xl border-t border-white/5 flex items-center px-6 gap-6 z-30">
      {/* Track Info */}
      <div className="w-1/4 min-w-[240px] flex items-center gap-4">
        <div className="w-14 h-14 rounded bg-white/5 border border-white/5 flex-shrink-0 overflow-hidden flex items-center justify-center group-hover:border-cyan-500/30 transition-colors">
          {currentTrack?.cover_image ? (
            <img
              src={getCoverSrc(currentTrack.cover_image)!}
        ...
        <div className="min-w-0">
          {currentTrack ? (
            <>
              <div className="font-bold text-white truncate text-base leading-tight" title={currentTrack.title}>
                {currentTrack.title}
              </div>
            ...
            <div className="text-gray-500 text-sm italic">No track playing</div>
          )}
        </div>
      </div>
